...
// Service page mapping to their translation object key
const SERVICE_PAGES = {
...
// We will collect the extracted Spanish subtitles
...
async function refactorPages() {
  ...
  for (const [file, i18nKey] of Object.entries(SERVICE_PAGES)) {
    ...
    // 1. Extract and replace subtitle
    // Looks like: subtitle="BASE DE DATOS Y INFRAESTRUCTURA"
    ...
    // 2. Replace Resultado Inmediato
    // Typically: <span className="...">Resultado Inmediato</span>
    // Note: React uses {} for expressions
    ...
    for (const [key, esSubtitle] of Object.entries(subtitles)) {
      ...
      if (regex.test(content)) {
        if (!content.includes(`subtitle: "${translatedSub}"`)) {
           // We might need to be careful if we run this multiple times
