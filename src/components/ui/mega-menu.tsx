    ...
    return (
      ...
      >
        {items.map((navItem) => (
          ...
          >
            <button
              className="relative flex cursor-pointer items-center justify-center gap-1 py-1.5 px-4 text-sm font-semibold text-white/80 transition-colors duration-300 hover:text-white group"
            ...
            <AnimatePresence>
              {openMenu === navItem.label && navItem.subMenus && (
                <div className="absolute left-0 top-full w-auto pt-2 z-10 pointer-events-auto">
                  <motion.div
                    className="w-max max-w-[calc(100vw-2rem)] md:max-w-none overflow-x-auto no-scrollbar border border-white/10 bg-[#0A0A0A] p-4 shadow-xl"
                    style={{
                      borderRadius: 16,
                    }}
                    layoutId="menu"
                  >
                    <div className="flex w-fit shrink-0 space-x-6 md:space-x-9">
