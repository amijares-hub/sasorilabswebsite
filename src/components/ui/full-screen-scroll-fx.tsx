...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-fixed-height" as any]: `${Math.max(1, total + 1) * 100}vh`,
    };
    ...
    return (
      ...
      >
        ...
        <div className="fx-scroll">
          <div className="fx-fixed-section" ref={fixedSectionRef} style={{ height: "var(--fx-fixed-height)" }}>
                ...
                {sections.map((s, i) => (
                  <div className="fx-bg" key={s.id ?? i}>
                    {s.renderBackground ? (
                      ...
                      <>
                      <img loading="lazy" ref={(el) => { if (el) bgRefs.current[i] = el; }} src={s.background} alt="" className="fx-bg-img" />
                    ...
                    <div className="fx-track" ref={leftTrackRef}>
                      {sections.map((s, i) => (
                        <div
                          key={`L-${s.id ?? i}`}
                          role="listitem"
                          className={`fx-item-wrapper ${i === index ? "active" : ""}`}
                        >
                          <div
