...
import { 
  Eye, Scissors, UserCheck, BarChart2,
  Briefcase, Zap, ShieldCheck, DollarSign
} from "lucide-react";
import { motion } from "framer-motion";
import { translations } from "../i18n/translations";

export function FinopsPage({ lang = "es" }: { lang?: string }) {
  const navigate = useNavigate();
  const t = translations[lang as keyof typeof translations] || translations.es;
  const tp = (t.servicesPages as any).finops;
            ...
            {tp.catalogItems.map((service, i) => (
              <div key={i} className="group relative bg-[#F8F8F8] p-8 rounded-2xl border border-black/5 overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  {i === 0 && <Eye className="w-24 h-24 text-sasori-red" />}
                  {i === 1 && <Scissors className="w-24 h-24 text-sasori-red" />}
                  {i === 2 && <UserCheck className="w-24 h-24 text-sasori-red" />}
                  {i === 3 && <BarChart2 className="w-24 h-24 text-sasori-red" />}
                </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              ...
              <DollarSign className="w-10 h-10 text-white/30 group-hover:text-sasori-red transition-colors duration-500 relative z-10" />
            ...
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <Eye className="w-10 h-10 text-white/30 group-hover:text-white transition-colors duration-500 relative z-10" />
              <div className="relative z-10">
            ...
            >
              <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/4 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity duration-700 pointer-events-none">
                <DollarSign className="w-[400px] h-[400px]" />
              </div>
              <div className="relative z-10 max-w-lg">
                <DollarSign className="w-12 h-12 text-white/50 mb-8" />
