...
import { 
  Target, Boxes, BarChart3, TrendingUp,
  Briefcase, Zap, ShieldCheck, Factory
} from "lucide-react";
import { motion } from "framer-motion";
import { translations } from "../i18n/translations";

export function DataBiPage({ lang = "es" }: { lang?: string }) {
  const navigate = useNavigate();
  const t = translations[lang as keyof typeof translations] || translations.es;
  const tp = (t.servicesPages as any).dataBi;
      ...
      <section className="py-24 px-6 bg-[#F8F8F8] relative overflow-hidden border-t border-black/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            {[
              { icon: <Target className="w-8 h-8" />, title: tp.mapping, desc: tp.mappingDesc },
              { icon: <Boxes className="w-8 h-8" />, title: tp.integration, desc: tp.integrationDesc },
              { icon: <BarChart3 className="w-8 h-8" />, title: tp.logic, desc: tp.logicDesc },
              { icon: <TrendingUp className="w-8 h-8" />, title: tp.panel, desc: tp.panelDesc }
            ].map((item, i) => (
              <motion.div 
                key={i}
            ...
            {tp.catalogItems.map((service, i) => (
              <div key={i} className="group relative bg-[#F8F8F8] p-8 rounded-2xl border border-black/5 overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  {i === 0 && <Target className="w-24 h-24 text-sasori-red" />}
                  {i === 1 && <Boxes className="w-24 h-24 text-sasori-red" />}
                  {i === 2 && <BarChart3 className="w-24 h-24 text-sasori-red" />}
                  {i === 3 && <TrendingUp className="w-24 h-24 text-sasori-red" />}
                </div>
      ...
      <section className="py-32 px-6 bg-gradient-to-br from-[#0A0A0A] via-[#2A0005] to-[#000000] text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-sasori-red/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <Factory className="w-10 h-10 text-white/30 group-hover:text-sasori-red transition-colors duration-500 relative z-10" />
              <div className="relative z-10">
            ...
            >
              ...
              <BarChart3 className="w-10 h-10 text-white/30 group-hover:text-white transition-colors duration-500 relative z-10" />
              <div className="relative z-10">
            ...
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <Target className="w-10 h-10 text-white/30 group-hover:text-white transition-colors duration-500 relative z-10" />
              <div className="relative z-10">
            ...
            >
              ...
              <div className="relative z-10 max-w-lg">
                <BarChart3 className="w-12 h-12 text-white/50 mb-8" />
