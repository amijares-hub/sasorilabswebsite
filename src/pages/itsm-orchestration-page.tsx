...
import { 
  Network, CopyX, ActivitySquare, Headset,
  Briefcase, Zap, ShieldCheck, Database
} from "lucide-react";
import { motion } from "framer-motion";
import { translations } from "../i18n/translations";

export function ItsmOrchestrationPage({ lang = "es" }: { lang?: string }) {
  const navigate = useNavigate();
  const t = translations[lang as keyof typeof translations] || translations.es;
  const tp = (t.servicesPages as any).itsmOrchestration;
      ...
      <section className="py-24 px-6 bg-[#F8F8F8] relative overflow-hidden border-t border-black/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            {[
              { icon: <Network className="w-8 h-8" />, title: tp.mapping, desc: tp.mappingDesc },
              { icon: <CopyX className="w-8 h-8" />, title: tp.integration, desc: tp.integrationDesc },
              { icon: <ActivitySquare className="w-8 h-8" />, title: tp.logic, desc: tp.logicDesc },
              { icon: <Headset className="w-8 h-8" />, title: tp.panel, desc: tp.panelDesc }
            ].map((item, i) => (
              <motion.div 
                key={i}
            ...
            {tp.catalogItems.map((service, i) => (
              <div key={i} className="group relative bg-[#F8F8F8] p-8 rounded-2xl border border-black/5 overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  {i === 0 && <Network className="w-24 h-24 text-sasori-red" />}
                  {i === 1 && <CopyX className="w-24 h-24 text-sasori-red" />}
                  {i === 2 && <ActivitySquare className="w-24 h-24 text-sasori-red" />}
                  {i === 3 && <Headset className="w-24 h-24 text-sasori-red" />}
                </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              ...
              <Database className="w-10 h-10 text-white/30 group-hover:text-white transition-colors duration-500 relative z-10" />
              <div className="relative z-10">
            ...
            >
              ...
              <div className="relative z-10 max-w-lg">
                <Network className="w-12 h-12 text-white/50 mb-8" />
