...
import { 
  ...
  MessageSquare, FileText, Globe, Send, Calendar
} from 'lucide-react';
...
export function UserAccountPage({ lang = 'es' }: { lang?: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: projData } = await supabase.from('projects').select('*').eq('client_id', session.user.id);
      if (projData) {
      ...
      const { data: payData } = await supabase.from('payments').select('*').eq('client_id', session.user.id);
  ...
  // Calculate Project total progress based on milestones average
  ...
  const t = {
    es: {
      welcome: 'Bienvenido', member: 'CLIENTE PREFERENTE', overview: 'Panel', projects: 'Proyectos',
    ...
    en: {
      welcome: 'Welcome', member: 'PREFERRED CLIENT', overview: 'Dashboard', projects: 'Projects',
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center">
        <MorphingSquare message="Sincronizando Dashboard..." />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans">
      ...
      <main className="pt-32 px-4 md:px-8 max-w-7xl mx-auto pb-20">
        {/* Header Profile... (omitted content from previous view for brevity but keeping structure) */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-12 bg-white/[0.02] border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl">
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 rounded-3xl bg-sasori-red/10 border border-sasori-red/20 flex items-center justify-center">
               {user?.full_name ? <span className="text-3xl font-black text-sasori-red">{user.full_name[0]}</span> : <User className="w-10 h-10 text-sasori-red" />}
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.4em] text-sasori-red mb-1">{t.member}</p>
              <h1 className="text-3xl font-black uppercase tracking-tighter">{user?.full_name || t.welcome}</h1>
                        ...
                        <div key={m.id} className="p-6 rounded-[2rem] bg-white/5 border border-white/10 group hover:border-sasori-red/30 transition-all">
                           ...
                           <div className="relative pt-4">
                              ...
                              <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                                 <motion.div initial={{ width: 0 }} animate={{ width: `${m.progress}%` }} className={cn("h-full rounded-full Shadow-red", m.progress === 100 ? "bg-emerald-400" : "bg-sasori-red")} />
                              </div>
                           </div>
                        </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 md:p-12 text-white overflow-hidden">
                 ...
                 <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-white/[0.03]">
                        {/* Mock data for demonstration, integration with real data would use a map */}
                        <tr className="group hover:bg-white/[0.01] transition-colors">
                          <td className="py-8 pl-4">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <p className="text-xs font-black uppercase mb-1 tracking-tight">Ajuste de Tipografía Premium</p>
                                   <p className="text-[8px] font-mono opacity-30">REF: RQ-7721-BC</p>
                                </div>
                             </div>
                          </td>
                          <td className="py-8">
                             <p className="text-[10px] font-black uppercase mb-1">{projects[0]?.name || 'Proyecto Principal'}</p>
                             <div className="flex items-center gap-2 opacity-40">
                                <div className="w-1 h-1 rounded-full bg-sasori-red" />
                                <p className="text-[9px] font-black uppercase">Fase de Implementación</p>
                             </div>
                          </td>
                          <td className="py-8 text-center">
                             <span className="px-3 py-1.5 bg-sasori-red/10 text-sasori-red border border-sasori-red/20 rounded-lg text-[8px] font-black uppercase tracking-widest">EN REVISIÓN</span>
                          </td>
                 ...
                 <div className="mt-12 pt-8 border-t border-white/5 flex items-center gap-4">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest text-white/20">Sistema sincronizado en tiempo real con la nube de Sasori</p>
                 </div>
              </div>
            </div>
          )}
      ...
      <AnimatePresence>
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-10">
                 ...
                 <button onClick={() => setIsRequestModalOpen(false)} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white/40 hover:text-white transition-colors">✕</button>
              </div>
              
              <form onSubmit={handleRequestSubmit} className="space-y-6 text-white">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-white/30 ml-2">Proyecto</label>
                      <select 
                        required
                        value={requestForm.projectId}
                        onChange={e => setRequestForm({...requestForm, projectId: e.target.value, milestoneId: ''})}
                        className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-[11px] font-black uppercase outline-none focus:border-sasori-red transition-all cursor-pointer"
                      >
                   ...
                   <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-white/30 ml-2">Fase (Hito)</label>
                      <select 
                        value={requestForm.milestoneId}
                        onChange={e => setRequestForm({...requestForm, milestoneId: e.target.value})}
                        className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-[11px] font-black uppercase outline-none focus:border-sasori-red transition-all disabled:opacity-30 cursor-pointer"
                        disabled={!requestForm.projectId}
                      >
                         <option value="" className="bg-black">General / Sin Hito</option>
                         {milestones.filter(m => m.project_id === requestForm.projectId).map(m => (
                ...
                <div className="space-y-2">
                   ...
                   <input type="text" placeholder="Título breve del cambio..." required 
                     className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-sm font-bold outline-none focus:border-sasori-red transition-all"
                     onChange={e => setRequestForm({...requestForm, title: e.target.value})} />
                </div>
                
                <div className="space-y-2">
                   <label className="text-[9px] font-black uppercase tracking-widest text-white/30 ml-2">Mensaje Detallado</label>
                   <textarea placeholder="Explícanos qué necesitas ajustar..." required rows={4}
                     className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-sm font-bold resize-none outline-none focus:border-sasori-red transition-all"
                     onChange={e => setRequestForm({...requestForm, description: e.target.value})} />
                </div>

                <button type="submit" 
                  className="w-full py-5 bg-sasori-red hover:bg-white text-white hover:text-sasori-red font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-[0_0_30px_rgba(226,6,19,0.2)]">
                  PROCESAR SOLICITUD →
                </button>
              </form>
            </motion.div>
        ...
        {isNewProjectModalOpen && (
          <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/98 backdrop-blur-2xl" onClick={() => setIsNewProjectModalOpen(false)} />
            <motion.div 
            ...
            >
              <div className="flex justify-between items-start mb-12">
                 <div>
                    <h2 className="text-4xl font-black uppercase tracking-tighter text-white mb-2 leading-none">Nueva Visión</h2>
                    <p className="text-[10px] font-black uppercase tracking-widest text-sasori-red">Sasori Labs Project Inquiry v1.0</p>
                 </div>
                 <button onClick={() => setIsNewProjectModalOpen(false)} className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center text-white/40 hover:text-white hover:bg-sasori-red transition-all">✕</button>
              </div>
              
              <form onSubmit={handleNewProjectSubmit} className="space-y-8 text-white">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-2">Nombre del Proyecto</label>
                      <input type="text" placeholder="Ej: Sasori Mobile App" required 
                        className="w-full bg-white/5 border border-white/10 rounded-3xl p-5 text-sm font-bold outline-none focus:border-sasori-red transition-all"
                        onChange={e => setNewProjectForm({...newProjectForm, projectName: e.target.value})} />
                   </div>
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-2">Sector / Industria</label>
                      <input type="text" placeholder="Ej: Fintech, Ecommerce..." required 
                        className="w-full bg-white/5 border border-white/10 rounded-3xl p-5 text-sm font-bold outline-none focus:border-sasori-red transition-all"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-2">Rango de Inversión</label>
                      <select 
                        className="w-full bg-white/5 border border-white/10 rounded-3xl p-5 text-[12px] font-black uppercase outline-none focus:border-sasori-red transition-all"
                   ...
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-2">Fecha Objetivo</label>
                      <input type="text" placeholder="Ej: Q3 2026" required 
                        className="w-full bg-white/5 border border-white/10 rounded-3xl p-5 text-sm font-bold outline-none focus:border-sasori-red transition-all"
                ...
                <div className="space-y-3">
                   <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-2">Objetivo del Proyecto</label>
                   <textarea placeholder="Cuéntanos tu visión, problemas a resolver y metas..." required rows={4}
                     className="w-full bg-white/5 border border-white/10 rounded-[2rem] p-6 text-sm font-bold resize-none outline-none focus:border-sasori-red transition-all"
                     onChange={e => setNewProjectForm({...newProjectForm, description: e.target.value})} />
                </div>

                <div className="p-8 rounded-[2.5rem] bg-sasori-red/5 border border-sasori-red/20">
                   <label className="text-[10px] font-black uppercase tracking-widest text-sasori-red block mb-4">Agendar Llamada de Análisis</label>
                   <div className="flex items-center gap-4">
                      <Calendar className="w-6 h-6 text-sasori-red" />
                      <input type="text" placeholder="Ej: Lunes 15:00h - GMT-5" required 
                        className="flex-1 bg-transparent border-none text-white font-black text-sm outline-none placeholder:text-white/20"
                        onChange={e => setNewProjectForm({...newProjectForm, callTime: e.target.value})} />
                   </div>
                </div>

                <button type="submit" 
                  className="w-full py-6 bg-sasori-red hover:bg-white text-white hover:text-sasori-red font-black uppercase tracking-[0.3em] rounded-3xl transition-all shadow-[0_0_50px_rgba(226,6,19,0.3)] text-xs">
                  ENVIAR PROPUESTA A ANÁLISIS →
                </button>
              </form>
            </motion.div>
          </div>
